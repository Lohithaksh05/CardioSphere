...
import {
  ...
  ShieldAlert, Smile, Activity, Stethoscope, ChevronRight,
} from "lucide-react";
...
const steps = [
  { title: "Basic Info", desc: "Personal details", fields: ["sex", "age", "bmi"] },
  { title: "Health Status", desc: "Current conditions", fields: ["high_bp", "high_cholesterol", "cholesterol_check", "diabetes", "stroke"] },
  { title: "Lifestyle", desc: "Daily habits", fields: ["smoker", "physical_activity", "fruits", "veggies", "heavy_alcohol"] },
  { title: "General", desc: "Overall health", fields: ["general_health", "difficulty_walking"] },
];
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl mx-auto space-y-6">
        ...
        <Card className={`${c.bg} ${c.border} border-2`}>
          <CardHeader className="text-center">
            ...
            <CardTitle className={`text-3xl ${c.text}`}>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-center">
              <div className="rounded-lg bg-white/60 p-4">
                ...
                <p className="text-xl font-bold">{(result.probability * 100).toFixed(2)}%</p>
              </div>
              <div className="rounded-lg bg-white/60 p-4">
                <p className="text-sm text-muted-foreground">Confidence</p>
                <p className="text-xl font-bold">{(result.confidence_score * 100).toFixed(1)}%</p>
              </div>
            </div>
            {rec?.summary && (
        ...
        <div className="flex gap-3 justify-center pb-8">
          <Button variant="outline" onClick={() => { setResult(null); setStep(0); setForm(initialForm); }}>
            New Assessment
          </Button>
          <Button asChild>
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Answer a few questions to predict your heart disease risk
        </p>
      </div>
      ...
      <div className="flex items-center gap-2">
        {steps.map((s, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            {i < steps.length - 1 && (
              <div className={`h-0.5 w-8 ${i < step ? "bg-emerald-500" : "bg-gray-200"}`} />
            )}
          </div>
        ))}
      </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {step === 0 && (
                <>
                  ...
                  <div className="space-y-2">
                    <Label>BMI</Label>
                    <Input
                      type="number" placeholder="e.g. 25.4" min={10} max={80}
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={form.diabetes} onValueChange={(v) => updateField("diabetes", v)}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="1">Pre-diabetes</SelectItem>
              ...
              {step === 3 && (
                <>
                  <div className="space-y-2">
                    <Label>General Health (1=Excellent to 5=Poor)</Label>
                    <Select value={form.general_health} onValueChange={(v) => updateField("general_health", v)}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="2">Very Good</SelectItem>
                        <SelectItem value="3">Good</SelectItem>
                        <SelectItem value="4">Fair</SelectItem>
                        <SelectItem value="5">Poor</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
          ...
          <div className="flex justify-between pt-6">
            ...
            {step < steps.length - 1 ? (
              ...
              <Button onClick={handleSubmit} disabled={loading} className="gap-2">
                {loading ? (
                  <><Loader2 className="h-4 w-4 animate-spin" /> Analyzing &amp; generating recommendations...</>
                ) : (
                  <><Heart className="h-4 w-4" /> Predict Risk</>
                )}
              </Button>
            )}
          </div>
